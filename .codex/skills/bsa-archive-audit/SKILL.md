...
description: Use for Skyrim project-local .bsa/.ba2 archives when Codex needs to audit archive contents, classify translatable resources, safely invoke BSAFileExtractor for BSA through the project wrapper, generate archive_audits manifests, or explain BSA/BA2-related QA blockers. Do not use for RAR, plugin editing, translation, GUI tools, BA2 extraction, or repacking archives.
---
...
Handle `.bsa/.ba2` as auditable archive boundaries. This Skill reads project-local BSA/BA2 archives only to produce evidence for routing and QA. BSA may be extracted through the project safe wrapper when materialization is required; BA2 is read-only audit only until a separate BA2 extractor adapter exists. This Skill does not translate files, edit archives, repack BSA, or install anything into Skyrim/MO2/Vortex.
...
3. `scripts/new_archive_audit_manifest.py` only after a project-local extraction directory exists.
4. Blocked report if neither read-only audit evidence nor safe extraction can be produced.

`BSAFileExtractor.py` is not called directly. Use only the project wrapper configured by `DecoderTools.BsaFileExtractorPath`; the wrapper must keep input under the project root and output under `work/archive_extracts/`. Do not use the BSA extractor wrapper for BA2.

## Workflow

1. Confirm the `.bsa/.ba2` path is inside the project and normally comes from `mod/` or `work/extracted_mods/<ModName>/`.
...
```console
python scripts/new_bsa_archive_manifest.py --mod-name <ModName> --archive-path <project-local-bsa>
```

4. If BSA files must be materialized, run:

```console
python scripts/invoke_bsa_file_extractor_safe.py --archive-path <project-local-bsa> --output-dir work/archive_extracts/<ModName>/<ArchiveName>
```

5. If BSA extraction was required, generate or refresh the extraction-backed manifest:

```console
python scripts/new_archive_audit_manifest.py --mod-name <ModName> --archive-path <project-local-bsa> --extracted-dir work/archive_extracts/<ModName>/<ArchiveName> --force
```
...
Use `Archive` as the archive filename or project-relative archive path. `Status` must be `accepted`, `approved`, or `exempted`; `Reason` and `Reviewer` are required. Optional `EvidencePath` must point to an existing project-local file.
...
- Do not claim complete localization if a BSA/BA2 exists and no project-local audit manifest exists.
- Do not extract or repack `.ba2`; BA2 still needs a separate adapter for materialization or writeback.

## Done When

- The BSA/BA2 path and any extraction output are project-local.
