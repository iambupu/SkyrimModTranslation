...
description: "用于安装/重新安装/注册/刷新这个 Bethesda Mod 汉化 Codex 插件。中文触发：帮我安装这个插件、安装 Codex 插件、重新安装插件、刷新本地 marketplace、检查插件安装、插件没装好。Covers marketplace registration, plugin validation, reinstall/cache refresh, and checking that Codex points at the current installed plugin source. Supports the Skyrim SE/AE stable and Fallout 4 Experimental capability set. Do not use for Mod translation, routing, QA, or final_mod assembly."
---

# Skyrim Mod CHS Install

Windows 环境下的 Bethesda Mod 简体中文汉化插件安装与注册指南。

## Scope

Use this Skill only for installing, registering, or validating this repository as the `skyrim-mod-chs-translation` Codex plugin source. The installed capability set supports Skyrim SE/AE and Fallout 4 Experimental through explicit Game Profiles.
...
Users do not need to understand Codex marketplace layout. Treat requests such as "帮我安装这个 Bethesda Mod 汉化 Codex 插件", "安装这个 Codex 插件", "重新安装插件", or "刷新本地 marketplace 入口" as plugin installation intent.
...
The plugin manifest description must identify this as a Windows Bethesda Mod Simplified Chinese localization workflow with Skyrim SE/AE stable support and Fallout 4 Experimental support. Keep `.codex-plugin/plugin.json` and Claude marketplace metadata aligned; marketplace files should not invent a different capability scope.
