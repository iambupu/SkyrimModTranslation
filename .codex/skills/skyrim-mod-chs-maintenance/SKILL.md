...
- Keep initialization tool setup explicit in both scripts and Skills: `--tool-setup auto` prepares safe non-GUI tools, `manual` writes reports/checklists only, and `skip` defers setup. Do not let GUI/system tools install silently. Auto mode must publish or reuse immutable machine-shared Python, pinned .NET SDK, hash-verified decoder source, and source-keyed adapter generations under the versioned Windows Local AppData managed store, then atomically bind the workspace through `.workflow/managed-tools.json`. The Python path may prefer uv copy mode but must fall back to standard `venv` and `pip`; both paths consume the committed exact hash-pinned runtime export. User external paths remain in `config/tools.local.json`; managed cache paths must not be written there. BSA extraction remains configured through `scripts/invoke_bsa_file_extractor_safe.py`, never the third-party extractor directly.
- Treat proven workspace-local legacy tools only as copy-only auto-migration seeds. Diagnostics and runtime wrappers must not execute them as a fallback, and an existing unsafe, stale, or unavailable binding must fail closed instead of silently selecting bootstrap Python or a legacy workspace venv.
- Keep shared-cache maintenance explicit and user initiated. Ordinary setup, doctor, translation, recovery, QA, release, upgrade, or workspace deletion must never clean the cache. `managed-tool-cache-maintenance` must enforce inspect -> plan -> exact confirmation -> apply -> inspect, keep live leases safe, and never delete arbitrary paths or user workspaces.
