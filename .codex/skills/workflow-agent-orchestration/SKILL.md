...
- Scripts own reproducible actions: project-local Python entries perform extraction, rebuilds, QA gates, state refreshes, and logging.
...
- missing BSA/BA2 audit manifest when the archive is project-local and a read-only audit is available
- missing loose override evidence when the source translated file already exists project-locally
...
- GUI save failures or any GUI action without project-local output evidence
...
Each row must stay project-local and must not contain hidden reasoning, credentials, or real game/MO2/Vortex paths.
