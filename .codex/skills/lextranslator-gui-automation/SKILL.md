...
description: Use only after routing selects LexTranslator GUI for Skyrim ESP, ESM, ESL, MCM, PEX, dictionary import, export, apply, or save into project-local tool_outputs. Do not use to decide translatable strings or edit binaries directly.
---
...
只负责 LexTranslator 的工具操作：启动、连接窗口、打开项目内文件、导入或应用项目内翻译材料、保存到项目内输出目录、记录日志。工具优先级由 `translation-task-router` 决定，翻译范围由文件类型 Skill 决定。
...
- PEX 输入必须是项目内副本，例如 `out/<ModName>/tool_outputs/Scripts/<ScriptName>.pex`。
