...
| `DecoderTools.Ba2ExtractorPath` | 实现受控 BA2 协议的审查过 adapter |
...
Fallout 4 PEX Export 可用，类别必须是 `Fallout4`。PEX Apply 是 Experimental：只有显式 opt-in、受控输出验证和 strict gate 都满足时才允许推进。缺少真实认证证据时，strict 完成仍会阻断。
...
| PEX Apply 未认证 | 补齐 opt-in 与 strict 证据，无法满足则保持 blocked |
