...
Skyrim SE/AE 与 Fallout 4 的 STRINGS/DLSTRINGS/ILSTRINGS 当前都是 `inventory_only` 或更低能力：可以识别和清点，但没有可验证的导出、写回、验证 adapter。xTranslator GUI 产物本身不能替代 adapter receipt 和最终文件 provenance，因此严格 QA 会保持阻断。
