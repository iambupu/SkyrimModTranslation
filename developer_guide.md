...
本文面向维护者和有技术经验的开发者，说明本项目的工作流设计理念、核心分层、状态机、扩展方式和最小验证要求。普通用户可以只看根目录 `README.md`；Codex/agent 接手流程单独见 `docs/codex_workflow.md`。

## 设计理念

本项目的核心目标不是“尽快改出一个能看的文件”，而是建立可维护、可回滚、可复核的 Skyrim Mod 汉化工程流程。

从架构上看，当前项目更准确地说是一个由状态机约束、多个反馈循环驱动的 agent 工作流。状态机不直接执行全部流程，而是记录阶段、授权动作、证据要求、下一步建议和停止条件；agent 和 Python 入口在这些约束内执行具体动作。外层接手循环负责按 Mod、阶段和阻断状态持续推进；文件类型循环负责发现、路由、抽取、翻译和写回不同资源；QA 循环负责在每次组装或恢复后重新生成证据，并决定是否允许进入下一阶段。
...
- 二进制文件只能由受控工具生成项目内副本，Codex 不直接修改。
...
| ESP/ESM/ESL | 导出文本、翻译中间表、由受控 Mutagen/xEdit adapter 写回项目内副本 |
...
| 位置 | 需要更新 |
|---|---|
| `.codex/skills/translation-task-router/SKILL.md` | 风险等级、推荐工具、输出目录和是否允许模型编排层直接处理 |
...
- `.codex/skills/workflow-policy-and-state/SKILL.md`
- `.codex/skills/workflow-agent-orchestration/SKILL.md`
