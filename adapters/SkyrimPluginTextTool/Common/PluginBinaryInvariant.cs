    ...
    private sealed record BinarySnapshot(List<ElementSnapshot> Elements)
    {
        ...
        private static void ParseElements(byte[] bytes, int start, int end, int depth, List<ElementSnapshot> elements)
        {
            ...
            while (offset < end)
            {
                ...
                if (signature == "GRUP")
                {
                    var rawSize = BinaryPrimitives.ReadUInt32LittleEndian(bytes.AsSpan(offset + 4, 4));
                    if (rawSize < 24 || rawSize > end - (long)offset)
                    {
                ...
                var rawDataSize = BinaryPrimitives.ReadUInt32LittleEndian(bytes.AsSpan(offset + 4, 4));
                var dataStart = offset + 24;
                if (rawDataSize > end - (long)dataStart)
                {
                    throw new InvalidDataException($"invalid record size at 0x{offset:X}");
                }
                ...
                if ((flags & CompressedRecordFlag) != 0)
                {
                    ...
                    var rawExpectedSize = BinaryPrimitives.ReadUInt32LittleEndian(recordData.AsSpan(0, 4));
                    if (rawExpectedSize > int.MaxValue)
                    {
                        throw new InvalidDataException($"compressed record size is too large at 0x{offset:X}");
                    }
                    var expectedSize = (int)rawExpectedSize;
