...
- 交付态 Interface 文本必须符合当前 Game Profile 的编码和运行时结构。Skyrim SE/AE 的 `Interface/translations/*.txt` 必须是 UTF-16 LE BOM；Fallout 4 由对应 profile validator 判定，不能照搬 Skyrim 编码。UTF-8/无 BOM 中间文件不能直接视为完成品。
- Fallout 4 localized plugin 使用的 `STRINGS`、`DLSTRINGS`、`ILSTRINGS` 不属于普通 loose text，必须交回 router 并 blocked。
