...
2. 对单个 Mod 的完整非 GUI 流程，运行 `python scripts/run_non_gui_translation_workflow.py`。需要排错或局部重跑时再执行下面的分步脚本。总控任一步骤失败必须走统一失败出口：写当前阶段失败报告，串行刷新 readiness、workflow state、tasks 和 Codex handoff，生成并输出当前 `[SMT 阻断]` 进度卡，然后返回非零退出码；刷新失败不得覆盖最初根因。
...
8. 对 ESP/ESM/ESL、PEX 和归档，按 profile capability 使用 decoder/CLI。Skyrim/Fallout 4 的 `.esl` 或带 light trait 的插件只允许只读导出，受控写回固定 blocked；FO4 localized plugin/STRINGS 也必须 blocked。普通插件只使用受控字段合同生成可写候选。PEX Export 可按 profile 执行，Apply 只有在 capability 允许时执行。BSA 交给 `bsa-archive-audit`；BA2 交给 `ba2-archive-audit`。归档译文默认作为同路径 loose override，BA2 不重打包。
...
19. 所有进入 final_mod 的译文默认必须以原相对路径和原文件名直接替换原文件；旁挂语言文件只作为中间件，除非 QA 证明游戏会加载它。
...
22. 调用 `final-mod-assembly` 生成完整 Mod 目录、中间产出汇总目录、必备 `translation_text_dictionary/` 翻译文本词典和 `_CHS.zip` 包。
