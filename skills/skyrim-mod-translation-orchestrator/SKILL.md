...
8. 对 ESP/ESM/ESL、PEX 和归档，按 profile capability 使用 decoder/CLI。Skyrim/Fallout 4 的 `.esl` 或带 light trait 的插件只允许只读导出，受控写回固定 blocked；两个游戏的 STRINGS 家族以及 FO4 localized plugin 也必须 blocked。普通插件只使用受控字段合同生成可写候选。PEX Export 可按 profile 执行，Apply 只有在 capability 允许时执行。BSA 交给 `bsa-archive-audit`；BA2 交给 `ba2-archive-audit`。归档译文默认作为同路径 loose override，BA2 不重打包。
