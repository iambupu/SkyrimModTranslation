...
准备 Mod 输入阶段：只读扫描工作区 `mod/`，识别文件类型和风险；工作区内 `.zip` 和可用 Python/CLI 适配器支持的 `.7z` 先解压到 `work/extracted_mods/<ModName>/`；发现 `.bsa/.ba2` 时交给 `bsa-archive-audit` 做只读归档审计，其中只有 BSA 可在必要时走受控 BSAFileExtractor wrapper，BA2 未配置 adapter 前不解包；RAR 只生成提取建议。此 Skill 不翻译、不调用 LexTranslator/xTranslator、不修改原始归档或插件。
...
- `.ba2/.rar` 默认不解包，除非后续添加明确的工作区安全 adapter；`.ba2` 仍应先生成只读审计 manifest。
