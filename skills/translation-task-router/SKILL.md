...
| Skyrim `.strings/.dlstrings/.ilstrings` | 高 | `xtranslator-gui-automation` | Codex-only xTranslator STRINGS 流程 | LexTranslator 仅在路由指定时后备 | 否；不得当普通文本解码，非 Codex adapter 必须交回 Codex |
| Fallout 4 `.strings/.dlstrings/.ilstrings` | 阻断 | `manual-review` | 无已认证 adapter | 无 | blocked；不得引用不存在的 Skill 或回退 Skyrim 路径 |
