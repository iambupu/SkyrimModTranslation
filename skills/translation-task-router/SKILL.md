...
description: "用于任何 Bethesda Mod 文件处理前的 Game Profile 分流和风险判断。中文触发：这个文件怎么处理、该用哪个工具、判断文件类型、路由文件、风险等级、能不能直接翻译、BA2、ESP、PEX、MCM。Use before processing files to select the profile-aware Skill, risk, adapter, and output. Routes STRINGS-family files to the dedicated adapter and localized plugins to profile-gated composite delivery. Do not translate, operate GUI, validate QA, or assemble final_mod."
---
...
| `.esp/.esm/.esl` | 高 | `esp-esm-esl-translation` | 当前 profile 声明的 plugin 或 localized composite adapter | Codex-only GUI fallback | 否；adapter 未实现、capability 不允许或必要证据缺失时 blocked |
| `.strings/.dlstrings/.ilstrings` | 高或阻断 | `bethesda-string-table-translation` | 当前 Profile 的 `bethesda-string-tables` adapter | 人工工具仅作对照 | Inventory/Export/Apply/Verify 分别受 capability 限制；不得回退为普通文本或由 GUI 提升能力 |
...
STRINGS-family 操作必须逐项服从 `capabilities.string_tables`：Inventory、Export、Apply、Verify 不能互相推高能力等级；GUI 可用性也不能提升该 capability。即使字符串表 Apply/Verify 已通过，没有 `localized_delivery` 复合证据时仍不得宣称 localized 插件交付完成。
...
- Light 插件写回按当前 Profile 和 master-style evidence 决定。STRINGS 家族固定走专用 adapter；localized 插件固定走 `localized_delivery`，当前两个 Profile 均为实验级。
...
- `.esp/.esm/.esl`、string table 和 `.pex` 按当前 Game Profile capability 选择 adapter；未声明或未实现的能力必须 fail closed。STRINGS 不得进入 generic text 或 GUI 提权路径；localized plugin 不得绕过 composite evidence，PEX Apply 不得越过 experimental opt-in 和 strict gate。
