...
| `.strings/.dlstrings/.ilstrings` | 高或阻断 | profile 声明的 string-table Skill；当前 Skyrim 为 `xtranslator-gui-automation` | 当前 profile 声明的 string-table adapter | Codex-only GUI fallback | `capabilities.string_tables` 不满足 read 时立即 blocked；不得回退到其他游戏 adapter 或当普通文本处理 |
...
- Skyrim 与 Fallout 4 的 `.esl` 或带 light trait 的插件只允许 inventory 和受支持的只读导出，不生成 Apply 产物。Fallout 4 localized 插件和 STRINGS 家族保持 blocked。
...
- `.esp/.esm/.esl`、string table 和 `.pex` 按当前 Game Profile capability 选择 adapter；未声明或未实现的能力必须 fail closed。FO4 localized plugin/STRINGS 必须 blocked，PEX Apply 不得越过 experimental opt-in 和 strict gate。
