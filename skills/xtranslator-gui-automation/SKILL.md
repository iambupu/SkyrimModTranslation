...
description: "用于 Codex-only xTranslator GUI 后备和精修。中文触发：xTranslator、查漏、精修、插件导入导出、PapyrusPex、GUI 后备。Use only after the current Game Profile and router explicitly select this real Skill with workspace-local paths. STRINGS-family and localized delivery use dedicated controlled adapters and never route here for capability evidence. Do not access real game paths or edit binaries directly."
---
...
- Skyrim 与 Fallout 4 的 `.strings/.dlstrings/.ilstrings` 不路由到本 Skill；它们使用专用 adapter。Localized 插件使用 composite adapter。缺少对应 adapter 或证据时必须 blocked，不得把 xTranslator GUI 输出当作受控交付证据。
