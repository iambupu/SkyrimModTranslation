...
description: "Skyrim SE/AE 默认完整入口与 Fallout 4 Experimental Support 的 Mod 简体中文汉化插件对外入口。中文触发：翻译 mod、汉化 mod、开始/继续汉化、初始化工作区、--game fallout4、准备工具、检查状态、进度卡、生成 final_mod、blocked 怎么办。Use first to read the workspace marker and Game Profile, recognize user intent, answer status/setup questions, and select the downstream Skill. Never infer the game from a Mod name. Do not sequence the runtime pipeline; delegate that to skyrim-mod-translation-orchestrator."
---
...
Use this as the Windows plugin entry point. Skyrim SE/AE remains the default complete workflow. Fallout 4 is exposed only as `Fallout 4 Experimental Support`. The workspace marker and resolved Game Profile are authoritative; never guess the game from a Mod name, archive name, or directory name. The plugin supplies rules, Skills, scripts, config templates, QA logic, glossary seeds, and workspace initialization. A workspace holds each run's `mod/`, `work/`, `qa/`, `out/`, `source/`, `translated/`, `glossary/`, and local tool config.
...
Users may ask for workspace initialization in natural language instead of naming the script. Treat requests such as "帮我初始化一个新的汉化工作区", "在 D:\SkyrimCHS\MyMod 创建工作区", "一键准备一个天际 Mod 汉化项目" or "创建 Fallout 4 实验性汉化工作区" as initialization intent. Extract the target path and explicit game intent; use `--game fallout4` only when the user selects Fallout 4. If no target path is provided, ask for it. Do not infer the game from the Mod name or create a workspace under the plugin repository.
