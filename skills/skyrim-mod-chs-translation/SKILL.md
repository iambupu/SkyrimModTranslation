...
顶层 Agent 必须读取 JSON 的 `outcome`、`workspace`、`mod_name`、`game_id`、`workflow_state`、`next_action.kind`、`next_action.summary`、`next_action.artifacts`、可选 `next_action.usage_id` 和 `diagnostics`。`next_action.artifacts` 指定的工作区内路径才是候选、校对包或其他动作输入；没有同名顶层字段。若 outcome 是 `needs_agent_translation`，处理指定路径；当权威 action 结构化声明需要模型动作时，有效 pending 可把 `usage_id` 作为非阻断元数据附到该 action。当前 Agent 支持记录时，可以运行 `python scripts\model_usage.py record --usage-id <usage_id> --status completed --output <output_path> --tool <tool>`，失败或取消分别记录 `failed`/`cancelled` 且不传 output。是否记录、记录失败或辅助状态损坏都不影响随后调用 `resume`，不得据此阻断翻译、QA 或 final_mod。若为 `needs_gui`，只有 Codex 可以执行获授权 GUI 动作，然后调用 `resume`；若为 `needs_user_input`，取得明确输入后调用 `resume`。`status` 只读最近状态快照，不签发或投影独立模型记录任务；`doctor` 只做诊断，`output` 只读公开产物路径。
