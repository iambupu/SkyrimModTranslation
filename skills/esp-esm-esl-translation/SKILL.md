...
- Skyrim 与 Fallout 4 的 `.esl`、带 light trait 的 `.esp/.esm` 和实际目标属于 Light owner 的记录当前按 `experimental_write` 处理。Apply 必须具备目标所需的 master-style context、canonical owner ModKey/local ID、精确 occurrence 和 source drift 证据；仅引用 Light master 不降低普通 full 插件自身目标的能力。
- 完整插件阶段先只读导出 TES4 候选并应用 translation map。只有 `target` 非空且不同于 `source` 的实际写回行 owner style 为 `unknown` 时，才针对该 owner 补齐 master-style 证据、重跑导出并重新应用 map；未翻译候选和无关第三方 master 缺失不得阻断，也不得被读取或哈希。`config/plugin_master_styles.json` 中的官方 Full master 直接使用版本化策略证据，不查找游戏文件；其他实际目标 `.esp/.esm` 的证据可由插件同目录或 `work/master_context/<game_id>/` 提供。任何路径都不得要求用户复制 `Skyrim.esm`、`Update.esm`、`Fallout4.esm` 等游戏本地文件。同名插件的 manifest 必须使用阶段生成的 ArtifactKey 路径。
