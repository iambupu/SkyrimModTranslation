...
def write_inventory(root: Path, scan_root: Path, report_path: Path, files: list[Path]) -> None:
    ...
    lines.append(
        f"| `Interface/translations/*.txt` | {len(interface_files)} | .codex/skills/text-resource-translation | Codex Text Pipeline |"
    )
    lines.append(f"| `MCM related` | {len(mcm_files)} | .codex/skills/mcm-translation | Codex Structured MCM Extractor |")
