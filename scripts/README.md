...
仓库根目录提供 `pyproject.toml`，方便使用 uv 运行脚本：

```powershell
uv run scripts\init_opencode.py D:\SkyrimCHS\YourWorkspace
```

uv 是可选便捷路径；所有脚本仍支持 `python scripts\...` 直接运行。工作区自动工具准备会在检测到 uv 时优先用 `uv venv` 和 `uv pip install` 创建 `tools/python-venv/`，失败时回退到 `venv + pip`。
...
| `init_opencode.py` | 为工作区写入 opencode 配置、刷新 handoff/context，并可一键启动 opencode。 |
...
| `init_opencode.py` | opencode 显式 adapter 辅助入口；生成工作区 `.opencode/` 配置和 `latest.opencode.context.md` 后启动 opencode。 |
