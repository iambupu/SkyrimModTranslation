...
工作区 `glossary/lextranslator_dynamic_dictionaries/` 和 `work/glossary_rag/` 是翻译辅助词典与检索索引，不是 `final_mod/` 的一部分，也不替代 `intermediate/translation_text_dictionary/`。动态词典命中包可以进入 `out/<ModName>/汉化产出/intermediate/` 作为参考证据，但 `glossary/` 本身不得打入 `_CHS.zip`。
