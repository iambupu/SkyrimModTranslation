...
- Skyrim 与 Fallout 4 的 `.esl`、带 light trait 插件或实际目标属于 Light owner 时按 `experimental_write` 处理；`current_plugin_light`、`references_light_master`、`targets_light_owner` 分别记录当前插件、依赖和实际目标，能力降级只看当前插件与目标。Apply/Verify 只确认目标所需的 master style。`config/plugin_master_styles.json` 中的官方 Full master 使用嵌入适配器的版本化证据，不要求用户复制 `Skyrim.esm`、`Update.esm`、`Fallout4.esm` 等游戏本地文件。实际目标为其他未知 `.esp/.esm` 时，才需要工作区 header 或由路径、SHA256 和 Small flag 绑定的 schema v2 manifest；未知、陈旧或冲突证据分别以稳定错误码阻断。
- 完整插件阶段先执行只读 export，再应用 translation map；只有实际产生非空且不同于原文的写回行，其 owner style 仍为 `unknown` 时，才针对这些 owner 从插件同目录和 `work/master_context/` 的游戏/Mod 分区查找 master，生成按插件相对路径区分的 schema v2 manifest、重跑 export 并重新应用 map。未被翻译的候选和其他无关 master 不查找、不读取、不哈希；显式或旧 manifest 的无关条目也不会触发证据文件校验。目标级预检失败记录 `master_style_preflight_blocked`；没有未知实际写回 owner 时不要求 manifest。直接 adapter 调用仍可显式传入 `--master-style-manifest`，但只校验本次目标引用的条目。
...
- Localized plugin 联合交付：`scripts/invoke_bethesda_localized_delivery.py` 以 Mod 资源锁串行执行直接入口，绑定插件锚点、引用覆盖、语言、组件 AdapterResult、输入输出 hash 和 schema v3 composite receipt。Apply/Verify 从绑定 translation snapshot 重算 coverage 与规范复核行；复核输入包含全部插件引用 ID 和其他实际变化 ID，且逐字段绑定 `file/table_type/string_id/Source/Result/risk`。旧 receipt、派生摘要或 coverage report 与绑定输入不一致时必须阻断；generic plugin/string-table 路径不能单独满足该能力。
