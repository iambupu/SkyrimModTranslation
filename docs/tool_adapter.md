...
- Skyrim 与 Fallout 4 的 `.esl` 或带 light trait 插件按 `experimental_write` 处理；只有检测到 Light 上下文时，Apply/Verify 才必须确认当前插件和相关 masters 的完整 master-style map。`config/plugin_master_styles.json` 中的官方 Full master 使用嵌入适配器的版本化证据，不要求用户复制 `Skyrim.esm`、`Update.esm`、`Fallout4.esm` 等游戏本地文件。其他 `.esp/.esm` master 需要工作区内 header，或由路径、SHA256 和 Small flag 绑定的 schema v2 manifest 提供证据；未知、陈旧或冲突证据分别以稳定错误码阻断。只读 inventory/export 不要求补齐该写回证据。
- 完整插件阶段只对 `.esl` 或带 light trait 的插件尝试为仍有歧义的第三方 `.esp/.esm` 从插件同目录和 `work/master_context/` 的游戏/Mod 分区查找 master，生成按插件相对路径区分的 schema v2 manifest；官方已知 Full master 不触发文件查找或 SHA-256。预检失败不阻断只读 export；发现写回候选后才在翻译前记录当前插件的 `master_style_preflight_blocked`，不得拖到 Apply 才失败。没有写回候选或没有歧义 master 时不要求补齐 manifest；直接 adapter 调用仍可显式传入 `--master-style-manifest`。
