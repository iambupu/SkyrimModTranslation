...
- 完整插件阶段先执行只读 export；如果导出候选的 owner style 为 `unknown`，再只针对这些 owner 从插件同目录和 `work/master_context/` 的游戏/Mod 分区查找 master，生成按插件相对路径区分的 schema v2 manifest 并重跑 export。无关 master 不查找、不哈希；旧 manifest 的无关条目不会触发其文件读取或哈希，并会被裁剪。目标级预检失败记录 `master_style_preflight_blocked`；没有未知目标 owner 时不要求 manifest。直接 adapter 调用仍可显式传入 `--master-style-manifest`。
