...
LexTranslator 和 xTranslator 是本项目的外部 GUI 后备工具，不是项目核心卖点。Tool Adapter 的作用是让 Codex 能主动判断是否应该启动工具，并通过 CLI、GUI 自动化或 Computer Use 将输入路径、输出路径和日志记录限制在当前项目内。
...
优先投入方向是 Mutagen、xEdit/SSEDump 安全包装器、PEX string tool 等可日志化、可重跑、可 hash 校验的 adapter。GUI 只在 decoder/CLI 缺失、格式不支持或必须由 GUI 写回项目内副本时进入。
...
如果 LexTranslator 或 xTranslator 不支持可靠命令行参数，且 decoder/CLI 路径已经被证明不可用，Codex 可以使用 Computer Use 操作 GUI：打开项目内输入、执行翻译/导出、保存到项目内 `tool_outputs`。GUI 自动化必须记录日志、保存前后路径和 QA 结果。
