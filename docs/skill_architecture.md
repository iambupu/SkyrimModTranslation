...
GUI Skill 只给 Codex。opencode 和 Claude Code 只能使用非 GUI 流程；Claude Code marketplace 也必须排除 `lextranslator-gui-automation` 和 `xtranslator-gui-automation`。遇到 `gui:desktop`、LexTranslator/xTranslator GUI、Computer Use、pywinauto 或 UI Automation 任务时，必须 blocked，并交回 Codex。

## 权威路由

`skills/translation-task-router` 是当前权威路由 Skill。

路由先读取当前 Game Profile 的资源 capability，再按操作级别选择受控 adapter。`support_level` 只用于向用户概括整体支持成熟度；插件、PEX、归档和外部字符串表能否执行或通过严格 QA，必须由各自 capability 与本次交付的实际使用证据决定。
...
当前项目以根目录 `skills/` 为唯一权威运行 Skill 目录。这个目录直接服务 `skyrim-mod-chs-translation` Codex 插件检索和执行，也由 Claude Code marketplace 列出非 GUI Skill；不得在其他位置维护第二套运行 Skill 镜像，避免后续 agent 在两个来源之间反复查找或读到过期规则。
...
```text
Use for Game Profile-supported Interface/translations/*.txt and JSON text resources. Do not use for ESP/PEX binary writeback or GUI automation.
```

不推荐写法：

```text
Translate Bethesda mods.
```
