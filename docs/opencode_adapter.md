...
初始化脚本还会生成 `.opencode/skills/<SkillName>/SKILL.md`。这些文件只用于 opencode 原生 Skill 发现，正文指向插件源根目录 `skills/`，不会在工作区复制第二套运行规则。
...
- 增量写入 opencode 配置：保留已有 `opencode.json` 字段和 `.opencode/AGENTS.md` 用户内容，再加入项目配置、本地插件和 `.opencode/skills/` 发现指针。
- 刷新 readiness、workflow state、workflow tasks、Codex handoff 和 agent handoff；agent checkpoint 在最后生成。
...
恢复前可先检查 handoff 是否仍然有效：

```powershell
python "$env:SKYRIM_CHS_PLUGIN_ROOT\scripts\write_agent_handoff.py" --check-freshness
```

返回码 `0` 表示 checkpoint 未变化；返回码 `2` 表示输入、产物或核心 QA 状态已变化，需要重新运行 `init_opencode.py <Workspace> --no-launch` 刷新状态链和上下文。
