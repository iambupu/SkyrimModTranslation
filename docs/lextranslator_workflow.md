...
通用说明不构成 string-table GUI 认证。Skyrim/Fallout 4 STRINGS 固定走专用 adapter，localized plugin 固定走 composite adapter；不得因为 decoder 失败转入 GUI，GUI 输出也不能提升 capability。
