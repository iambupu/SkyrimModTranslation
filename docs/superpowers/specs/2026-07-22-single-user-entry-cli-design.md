...
- `scripts/smt_cli.py`：实现路径解析、指纹、工作区缓存、锁、事务导入、子进程监管、任务推进和结果投影。该模块不调用 `print`，只返回结构化结果。
...
-> run 输入的复合指纹映射
-> status/resume/doctor/output 的最近活动工作区
-> Known Folder 默认工作区根目录
```
...
```text
fingerprint-v1:<game_id>:<source_kind>:<sha256>
```

`source_kind` 第一版只允许 `zip`、`7z` 和 `directory`。相同归档使用不同 Game Profile 时不得复用。同一摘要来自不同来源类型时不得复用。指纹算法升级时必须使用新的算法标识，不能把新旧摘要混用。
...
归档以 1 MiB 分块流式计算文件 SHA-256。计算前后比较 `(st_dev, st_ino, st_size, st_mtime_ns)`；输入在计算期间变化时停止。归档本身也必须通过普通文件、链接/reparse 和多硬链接检查。
...
session、全局缓存、reservation 更新和导入失败报告都必须先在同一文件系统内写入临时文件，刷新内容后再用 `os.replace()` 原子提交。原子替换用于防止内容撕裂；跨进程互斥仍由 `SmtProcessFileLock` 保证。
...
进程异常退出后 reservation 记录可以保留用于诊断。后续命令若确认对应 reservation 锁当前无人持有且没有合法 session，不删除或复用原路径，而是把该记录视为未完成 reservation，并为新尝试分配其他名称。`doctor` 只报告，不修复。
...
2. 当前状态是 `ready_for_manual_test`：`ready_for_manual_test`；
...
outcome 分类读取 marker/session、workflow state、workflow tasks 和 progress card。编排控制还可以只读 `config/workflow_policy.json`，以取得 `max_same_blocker_attempts=2` 等权威策略。单次命令以 `blocker + evidence` 统计本次尝试；跨命令判断只使用 workflow state 已有的 `retry_count`、`last_attempt` 和 `stop_conditions`。CLI 不建立第二套持久化重试状态。
...
```json
{
  ...
  "state_generated_at": "<UTC ISO-8601>",
...
- `config/workflow_policy.json`：登记 `smt.py` 为公开控制入口，但不新增 workflow state；
