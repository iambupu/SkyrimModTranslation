...
finalize_mod_name(candidate: str, digest: str) -> FinalizedModName
...
RAR、单独 ESP/ESM/ESL/BSA/BA2 顶层输入必须抛 `UnsupportedInputError`。标准命名调用链必须为 `choose_workspace_name(finalize_mod_name(derive_mod_name_candidate(path), digest), digest, occupied)`；`choose_workspace_name(finalize_mod_name("Example", digest), digest, {"Example", "Example-01234567", "Example-01234567-2"})` 必须返回 `Example-01234567-3`；session、导入目标和工作区名称都不得超过 80 UTF-16 code units。
...
目标复制后重新构建完整 manifest 并比较 digest/entries；随后重新计算源目录全部文件 SHA-256 和完整 manifest/digest，除新增/删除/重命名外，还要以回归测试覆盖同长度覆写并恢复 mtime。归档目标验证后重新计算源 SHA-256 并再次比较 identity。名称先派生未截断安全候选，再由 `finalize_mod_name()` 按 UTF-16 code unit 收敛为带结构化摘要来源证据的 `FinalizedModName`，session/import 持久化其普通字符串 `.value`，最后才由 `choose_workspace_name()` 处理占用冲突；后者必须拒绝未收敛输入，也不得根据名称文本后缀猜测摘要来源。
