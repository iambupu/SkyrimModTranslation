...
在 `scripts/smt_cli.py` 使用 dataclass 定义 `CliResult`，字段与 `EXPECTED_KEYS` 一一对应；`schema_version=1`，缺值使用 `None`、空列表或空字典；`to_payload()` 使用 `dataclasses.asdict()`。模块导入和 `empty_result()` 不得 print。
...
目标复制后重新构建完整 manifest 并比较 digest/entries；随后重新枚举源目录的路径、类型、大小和 identity。归档目标验证后再次比较源 identity。名称先用 `safe_file_name()`，再按 UTF-16 code unit 安全截断。
...
fixture 子进程创建长驻孙进程并写 PID。超时后两个 PID 都必须退出，结果为 `124`，输出尾部最多 200 行。实现 Process Group + Job Object + `JOB_OBJECT_LIMIT_KILL_ON_JOB_CLOSE`；Ctrl+C 先发 CTRL_BREAK，再关闭 Job；Job 分配失败时终止已启动树并以 `taskkill /PID <pid> /T /F` 兜底，公开返回 `5`。
...
- [ ] 目录源复制后重新枚举，归档源在目标验证后再次验证 identity。
