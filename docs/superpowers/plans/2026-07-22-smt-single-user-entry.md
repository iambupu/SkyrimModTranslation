...
derive_mod_name_candidate(path: Path) -> str
finalize_mod_name(candidate: str, digest: str, *, source_kind: Literal["directory", "zip", "7z"]) -> FinalizedModName
choose_workspace_name(final_mod_name: FinalizedModName, digest: str, occupied: Collection[str]) -> str
```
...
RAR、单独 ESP/ESM/ESL/BSA/BA2 顶层输入必须抛 `UnsupportedInputError`。标准命名调用链必须为 `finalized = finalize_mod_name(derive_mod_name_candidate(path), digest, source_kind=manifest.source_kind)`，session 使用 `finalized.value`，导入目标直接使用 `finalized.import_name`，工作区调用 `choose_workspace_name(finalized, digest, occupied)`；不得由调用者自行拼接归档扩展名。`choose_workspace_name(finalize_mod_name("Example", digest, source_kind="directory"), digest, {"Example", "Example-01234567", "Example-01234567-2"})` 必须返回 `Example-01234567-3`；session、导入目标和工作区名称都不得超过 80 UTF-16 code units。
...
固定常量：`DIRECTORY_MAGIC=b"SMT-INPUT-DIR\x00"`、`DIRECTORY_VERSION=1`、`HASH_CHUNK_SIZE=1024*1024`、`FINGERPRINT_ALGORITHM="smt-input-v1"`。entry 编码依次包含类型 byte、UTF-8 路径长度、NFC POSIX 路径、uint64 size、文件 raw SHA-256。按规范化 UTF-8 bytes 排序；casefold key 重复即拒绝。每个文件哈希前后比较完整 `FileIdentity`。根目录和每个目录 entry 也必须保存 no-follow identity，并在遍历前、`scandir` 后和 manifest 返回前验证未替换；identity 证据不进入 digest。

- [ ] **Step 5: 实现双重验证和命名**

目标复制后重新构建完整 manifest 并比较 digest/entries；随后重新计算源目录全部文件 SHA-256 和完整 manifest/digest，除新增/删除/重命名外，还要以回归测试覆盖同长度覆写并恢复 mtime。归档目标验证后重新计算源 SHA-256 并再次比较 identity。名称先派生未截断安全候选，再由 `finalize_mod_name()` 根据 `source_kind` 为摘要和归档扩展名共同预留 UTF-16 预算，返回带结构化来源证据的 `FinalizedModName`；session 持久化普通字符串 `.value`，导入目标使用 `.import_name`，最后才由 `choose_workspace_name()` 处理占用冲突。后者必须拒绝未收敛输入，也不得根据名称文本后缀猜测摘要来源。
