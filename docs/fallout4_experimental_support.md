...
PEX Export 使用 `Fallout4` category，并按 `config/pex_visible_apis/fallout4.json` 解析调用目标、opcode 和语义参数位置。只有注册表授权的直接字面量标记为 `visible`；诊断、配置、协议值标记为 `protected`，未知调用和动态参数标记为 `manual_review`。文本是否像自然语言不能改变分类。
