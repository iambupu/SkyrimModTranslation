...
| `.bsa/.ba2` | 可行但需要 CLI 解包器 | 配置 `DecoderTools.BsaExtractorPath` / `Ba2ExtractorPath` | 未配置时只生成提取计划 |
...
1. Codex 结构化文本管线。
2. 项目内配置的 CLI/库 decoder。
3. LexTranslator/xTranslator GUI。
4. 人工操作清单。
...
这些报告集中列出核心脚本、`.codex/skills/`、final text/binary review packet、模型校对、严格门禁和 `final_mod` 证据。后续 agent 应优先读取它们，避免重复探索分散的 QA 文件；脚本化接手优先读 JSON。
...
当前还没有配置 BSA/BA2 的 CLI decoder，因此：
...
如果 Mod 含有 BSA/BA2，严格完成模式要求先获得项目内解包结果，再生成内容审计 manifest。当前项目不默认解包 BSA/BA2；解包器未配置时，只能阻断完整交付，不能假装归档内没有文本。

当归档已经由项目内工具解到 `work/` 下后，生成 manifest：
