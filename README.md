...
| ESL 及带轻量 FormID 的插件 | 实验性受控写回；官方 Full master 使用版本化策略，只有实际写回目标属于未知第三方 `.esp/.esm` 时才需目标级工作区证据 | 实验性受控写回；官方 Full master 使用版本化策略，只有实际写回目标属于未知第三方 `.esp/.esm` 时才需目标级工作区证据 |
