...
它会检查 Mod 中有哪些可翻译内容，选择对应工具，生成译文和检查报告，最后整理出可供 Mod 管理器测试的完整汉化包。原始 Mod 不会被直接改写，流程也不会访问真实游戏目录或自动修改 MO2/Vortex。
...
- `final_mod/`：保持游戏 Data 目录结构的完整汉化 Mod。
- `<ModName>_CHS.zip`：供 MO2、Vortex 等 Mod 管理器手动导入测试。

词典不是开始翻译的必要条件，但强烈推荐提供当前游戏的词典和 Mod 术语表。没有词典也能继续，术语一致性可能会下降。
...
| ESL 及带轻量 FormID 的插件 | 可读取和导出；暂不写回 | 可读取和导出；暂不写回 |
| STRINGS/DLSTRINGS/ILSTRINGS 外部字符串表 | 可由 Codex 配合 xTranslator 处理 | 暂不支持，检测到后暂停 |
